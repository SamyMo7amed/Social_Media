...
namespace Social_Media.InfraStructure.ImplementationRepositories.NotificationsRepository.AddFriendRequest_Notification
{
    ...
    {
        private readonly DbSet<ConfirmFriendRequestNotification> ConfirmFriendRequestNotification;
        private readonly ILogger Logger;
        public ConfirmFriendRequestNotificationRepository(Data.ContextData Data, ILogger Logger) : base(Data, Logger)
        {
            this.ConfirmFriendRequestNotification = Data.Set<ConfirmFriendRequestNotification>();
            this.Logger = Logger;
        }
        ...
        public async Task<ConfirmFriendRequestNotification> GetByNotificationId(int NotificationId)
        {
            try
            {
                return await ConfirmFriendRequestNotification.Where(CN => CN.NotificationId == NotificationId).FirstOrDefaultAsync();
            }
            catch (Exception ex)
            {
